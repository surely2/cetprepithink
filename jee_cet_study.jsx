...
export default function App() {
  ...
  const renderLatex = (latex) => {
    if (!katexLoaded || !window.katex) return <span className="font-mono text-sm">{latex}</span>;
    try {
  ...
  const renderLatexDisplay = (latex) => {
    if (!katexLoaded || !window.katex) return <span className="font-mono text-sm">{latex}</span>;
    try {
      const html = window.katex.renderToString(latex, { displayMode: true, throwOnError: false });
      return <span dangerouslySetInnerHTML={{ __html: html }} />;
    } catch {
      return <span className="font-mono text-sm">{latex}</span>;
    }
